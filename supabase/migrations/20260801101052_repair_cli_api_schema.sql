-- Repair the CLI API schema created by the pre-release DDL. The initial
-- production schema differs from the committed CLI implementation. These
-- tables must remain empty; fail closed instead of silently deleting data.

BEGIN;

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.cli_device_authorizations)
        OR EXISTS (SELECT 1 FROM public.cli_access_tokens)
        OR EXISTS (SELECT 1 FROM public.cli_idempotency_keys)
        OR EXISTS (SELECT 1 FROM public.cli_api_audit_logs) THEN
        RAISE EXCEPTION 'Refusing to repair CLI API schema because CLI data exists. Export data and write a data migration first.';
    END IF;
END;
$$;

DROP FUNCTION IF EXISTS public.cli_claim_idempotency_key(UUID, TEXT, TEXT);
DROP TABLE IF EXISTS public.cli_api_audit_logs;
DROP TABLE IF EXISTS public.cli_idempotency_keys;
DROP TABLE IF EXISTS public.cli_access_tokens;
DROP TABLE IF EXISTS public.cli_device_authorizations;
...
CREATE TABLE public.cli_idempotency_keys (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE TABLE public.cli_api_audit_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_cli_access_tokens_user_active
    ON public.cli_access_tokens (user_id, expires_at DESC)
    WHERE revoked_at IS NULL;

CREATE INDEX idx_cli_device_authorizations_user
    ON public.cli_device_authorizations (user_id, created_at DESC);

CREATE INDEX idx_cli_audit_logs_user_created
